...
# Includes Dynamic Path Routing, Locked Random Seeds, and Profiling Decomposition.
# ==============================================================================
...
# Automatically resolves the parent directory of /src to find /data and /results
...
class Advanced_RL_Tuner:
    def __init__(self, actions, stagnation_threshold=0.01, temperature=0.1):
...
def load_data(base_path):
    ...
    print(f"Loading data from '{base_path_str}'...")
    try:
    ...
    if os.path.exists(sp_len_path) and os.path.exists(sp_path):
        ...
        print("\033[93mCache not found, pre-calculating shortest paths between key network nodes...\033[0m")
...
def op_inter_route_relocate_stop(ind, **kwargs):
    ...
    new_r_dest = r_dest[:insert_pos] + path_seg_dest1[1:-1] + [stop_to_move] + path_seg_dest2[1:] + r_dest[insert_pos + 1:]
    ind_copy['routes'][idx_source], ind_copy['routes'][idx_dest] = list(dict.fromkeys(new_r_source)), list(dict.fromkeys(new_r_dest))
    return ind_copy
...
def op_worst_routes_regeneration(ind, **kwargs):
    sp, stop_nodes_set, od_df, sp_len, G = kwargs['sp'], kwargs['stop_nodes_set'], kwargs['od_df'], kwargs['sp_len'], kwargs['G']
    ind_copy = copy.deepcopy(ind)
    routes = ind_copy['routes']
...
def op_heuristic_swap_stops(ind, **kwargs):
    ...
    if not all(x > -1 for x in [idx1, i1, idx2, i2]) or idx1 == idx2 or i1 == 0 or i2 == 0 or i1 >= len(routes[idx1]) - 1 or i2 >= len(routes[idx2]) - 1:
        return ind
    r1_original, r2_original = routes[idx1], routes[idx2]
    prev1, next1 = r1_original[i1 - 1], r1_original[i1 + 1]
    path_seg1a, path_seg1b = sp.get(prev1, {}).get(s2), sp.get(s2, {}).get(next1)
...
def op_smart_shrink_route(ind, **kwargs):
    ...
    for r_idx in segment_loads:
        for (o, d), demand in segment_loads[r_idx].items():
            stop_usage[o] += demand; stop_usage[d] += demand
    eligible_routes = [i for i, r in enumerate(ind_copy['routes']) if r and len([s for s in r if s in stop_nodes_set]) > MIN_STOPS_PER_ROUTE]
    ...
    for target_od in unmet_ods:
        ...
        if closest_route_idx != -1:
            ...
            if score > best_extension['score']:
                ...
                best_extension = {'score': score, 'route_idx': closest_route_idx, 'new_route': list(dict.fromkeys(new_route))}
...
def run_genetic_algorithm(G, od_df, sp_len, sp, initial_solution_path, stop_nodes_set, algorithm_mode='ISA-MA',
    ...
    print(f"\n======== Starting {algorithm_mode} Optimization (Results saved to: {output_path}) ========")
    ...
    names_fine_tuning = ["Intra-Route Reroute", "Inter-Route Relocate", "Merge & Split", "Destroy & Rebuild"]
    ...
    names_restructuring = ["Heuristic Swap", "Heuristic Relocate", "Smart Shrink", "Smart Extend", "Merge & Regenerate", "Smart Split"]
    ...
    with Pool(os.cpu_count()) as pool:
        with tqdm(total=MAX_GENERATIONS, desc=f"Running {algorithm_mode}") as pbar:
            for gen in range(MAX_GENERATIONS):
                ...
                time_fitness_eval += time.time() - t_fit_start
                ...
                if algorithm_mode == 'GA':
                    ...
                    if algorithm_mode == 'ISA-MA' and rl_tuner:
                        ...
                        if gen > 0 and learning_records:
                            ...
                            learning_records = []
                        time_rl_control += time.time() - t_rl_start
                        ...
                        for i, child in enumerate(children_to_process):
                            ...
                            time_local_search += time.time() - t_ls_start
    ...
    if operator_stats:
        print("\n" + "=" * 25 + " Operator Performance Analysis Report " + "=" * 25)
        stats_list = []
    ...
    print(f"\n======== {algorithm_mode} finished. Total time: {total_time / 60:.2f} mins ========")
    if rl_tuner:
        ...
        pd.DataFrame(rl_tuner.q_table, columns=rl_tuner.actions, index=q_table_labels).to_csv(q_table_df_path, encoding='utf-8-sig')

        if algorithm_mode == 'ISA-MA':
            total_tracked = time_fitness_eval + time_local_search + time_rl_control
            print("\n" + "=" * 25 + " ISA-MA Runtime Decomposition " + "=" * 25)
            print(f"Total Tracked Time:          {total_tracked:.2f} s")
            print(f"Passenger Assignment (Fit):  {time_fitness_eval:.2f} s ({time_fitness_eval / total_tracked * 100:.2f}%)")
            print(f"Local Search Execution:      {time_local_search:.2f} s ({time_local_search / total_tracked * 100:.2f}%)")
            print(f"RL Control Overhead:         {time_rl_control:.2f} s ({time_rl_control / total_tracked * 100:.2f}%)")
            print("=" * 75 + "\n")
...
def plot_q_table_heatmap_revised(q_table, actions, states, filename):
    ...
    heatmap = sns.heatmap(
        q_table, annot=True, fmt=".3f", cmap="viridis",
        xticklabels=actions, yticklabels=states, linewidths=.5,
        linecolor='white', cbar_kws={"shrink": .82}, annot_kws={"size": 14, "weight": "bold"}
    )
...
def display_and_save_results(best_individual, history, G, od_df, sp_len, stop_nodes_set, output_path, rl_tuner=None,
    ...
    if rl_tuner:
        action_aliases = [f'a{i}' for i in range(len(rl_tuner.actions))]
        states_for_plot = ["Z0", "Z1", "Z2", "Z3"]
        plot_q_table_heatmap_revised(rl_tuner.q_table, action_aliases, states_for_plot, os.path.join(output_path, "q_table_heatmap.png"))
        if operator_history:
            plot_operator_selection_dynamics(operator_history, action_aliases, os.path.join(output_path, "operator_selection_dynamics.png"))
...
def main():
    ...
    print("======================================================")
    print("=   ISA-MA Optimization Algorithm Platform   =")
    print("======================================================")
    ...
    global NUM_ROUTES, MAX_TOTAL_FLEET, MAX_STOPS_PER_ROUTE, MIN_STOPS_PER_ROUTE
    NUM_ROUTES, MAX_TOTAL_FLEET, MAX_STOPS_PER_ROUTE, MIN_STOPS_PER_ROUTE = \
        config["num_routes"], config["max_fleet"], config["max_stops"], config["min_stops"]
    base_path_obj = config["path"]

    print("\nStep 2: Please select the algorithm mode to run:")
    print("  1. GA (Standard Genetic Algorithm)")
    print("  2. MA-Random (Memetic Algorithm with Random Local Search)")
    ...
    if not base_path_obj.exists():
        print(f"\nError: Base data path '{base_path_obj}' does not exist. Ensure you run this script from the correct directory structure.")
        return

    G, node_df, od_df, sp_len, sp, stop_nodes_set = load_data(base_path_obj)
    if G is not None and stop_nodes_set is not None:
        initial_solution_file = os.path.join(str(base_path_obj), config["initial_solution"])
        if not os.path.exists(initial_solution_file):
